...
**"Canva for data" is the north star, not the wedge.** It is where the product is heading,
and the right destination. Bricks already markets itself in that position, so the phrase on
its own does not differentiate; the wedge that gets there is *It catches what you'd have
missed* — silent failure, which nobody else sells. That split is settled: the positioning
leads with the catch, and leading with "Canva for data" is not reopened without a design
change.
...
| Ignoring an undeclared id in `withoutGuards` | Nobody types a `GuardId` — it comes off a trust report — so an unknown one means the client and the layer disagree. Running every check and reporting success is invariant 4's silent coercion arriving through a no-op instead of a nearest match. It is a 400 naming the id |
| Ranking language on a sequence, or withholding the block from one | The catch block exists to show a confident claim the data does not support; on a breakdown-ordered answer, "led with" makes one, because the first row is only the earliest member. Every clause reads `spec.sort.by`, as `ai/narrate-template.ts` already did. Declining to draw the block for sequences was the other option and hides a true catch — the mirror of manufacturing a false one (architecture §11) |
| Pluralising a declared label in surface copy | A label is layer data, and inflecting it breaks silently on the first locale whose plural is not `+s` — in the one sentence carrying the product's argument. Copy writes `{n} {label} values`, the idiom the trust strip already ships, and inflects only words the surface owns (`record`/`records`) |
